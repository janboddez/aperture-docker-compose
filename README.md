...
Alternatively, you could enter the Watchtower and Aperture containers and, once inside, run `git pull` and `composer install --no-dev`. In Aperture's case, `cd` into the `aperture` subfolder first before running `composer`. Make sure you do this as the user with uid 82 (or whatever uid belongs to `www-data` inside the containers), though, or properly chown everything in `html` (on the host) afterward. Otherwise, the containers' respective `www-data` users won't be able to move or write to files.
